...
// Loading flow :  Mount => render => update information (getPage->getMovies->get movie_covers->get number of all movies ->modify page -> re-render to apply all update to pages)
    ...
    componentDidMount = async() =>{
        console.log('mount')
        this.search_content = (await this.getPage())['content']
    ...
    getMovies =  async (Query) =>{
        ...
        await
        axios.get("http://localhost:3001/search?content="+this.search_content + "&page=" + page + "&num=" + this.state.max_movies_in_one_page)
                .then(res =>{
    ...
    get_number_of_movies = async() =>{
        ...
        var number_of_movies = await axios.get('http://localhost:3001/getNumberofMovies?content='+this.search_content)
    ...
    console.log('render')
        return(
                <div id='content' className="Movie" >
                        ...
                        <div id="bottom">
                            <div>
                                <Link to={{pathname:'/search',search:"?content=" + this.search_content}}>
                                    <button name="First" className="begin_and_end" > First </button>
                                </Link>
                                <Link to={{pathname:'/search',search:"?content=" + this.search_content  + (Number(this.state.page) === 2)? "" : "&page=" + (Number(this.state.page)-1)}}>
                            ...
                            <div>
                                { 
                                this.pages.map((elem=>
                                <Link to={{pathname:'/search',search:"?content=" + this.search_content + (elem === 1 ? "" : ("&page=" + elem))}}>
                            ...
                            <div>
                                <Link to={{pathname:'/search',search:"?content=" + this.search_content + "&page=" + (Number(this.state.page)+1)}}>
                                    <button name='Next' className="begin_and_end" > Next </button>
                                </Link>
                                <Link to={{pathname:'/search',search:"?content=" + this.search_content + "&page=" + this.max_page}}>
